...
public class UserServlet2 extends BaseServlet {
	...
	private Map<String,String> validateRegist(User formUser, HttpSession session) {
		...
		} else if(!loginname.contains("_")){
			System.out.println("hdsihafudiskghdsjfksaghkgfhgsadk");
			errors.put("loginname","格式应为：hospital_doctor");
		}
...
/*
	...
	public String login(HttpServletRequest req, HttpServletResponse resp)
		...
		System.out.println();
		if(errors.size() > 0) {
			req.getSession().setAttribute("form", formUser);
		...
		if(user == null) {
			...
			return "r:/jsps/user/login.jsp";//重定向到主页
		} else {
				// 保存用户到session
				...
				req.getSession().setAttribute("Operator", loginname);
				loginname = URLEncoder.encode(loginname, "utf-8");
				Cookie cookie = new Cookie("loginname", loginname);
				cookie.setMaxAge(60 * 60 * 24 * 10);//保存10天
				resp.addCookie(cookie);
				
				return "r:/qiye_admin/HTML_model/index.jsp";//重定向到主页
			}
		}
