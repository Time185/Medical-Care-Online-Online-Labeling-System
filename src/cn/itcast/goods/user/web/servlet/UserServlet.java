...
public class UserServlet extends BaseServlet {
	...
	public String regist(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 */
		String asdHospital=formUser.getLoginname().split("_")[0];
		File asdHospitalDir=new File(path+asdHospital);
		 if (!asdHospitalDir.exists()) {                           
			 asdHospitalDir.mkdir();     
	     }	
		File asdHospitalDir1=new File(path+asdHospital+"/maindirforemptydirexceptopn_neverwillbeRegisted");
		 if (!asdHospitalDir1.exists()) {                           
			 asdHospitalDir1.mkdir();     
	     }
		 System.out.println("ok");
		//req.setAttribute("code", "success");
		req.setAttribute("msg", "注册功能，请登陆！");
	...
	private Map<String,String> validateRegist(User formUser, HttpSession session) {
		 ...
		 * 4. 校验手机号
		 */
		String iphone = formUser.getIphone();
		if(iphone == null || iphone.trim().isEmpty()) {
			errors.put("iphone", "iphone不能为空！");
		} else if(iphone.length() != 11) {
			errors.put("iphone", "手机号格式错误！");
		} else if(!userService.ajaxValidateIphone(iphone)) {
			errors.put("iphone", "iphone已被注册！");
		}
		/*
		 * 5. 验证码校验
		 */
	...
	public String login(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException, ParseException, UserException {
		/*
		 * 1. 封装表单数据到User
		 * 2. 校验表单数据
		 ...
		 */
				...
				if(errors.size() > 0) {
					req.getSession().setAttribute("msg", errors.values());
				...
				if(user == null) {
					...
					return "r:/jsps/user/login.jsp";
						...
						////修改登陆日期
						userService.updateDate(user.getDate(),user.getUid());
